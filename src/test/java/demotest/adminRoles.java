...
public class adminRoles extends base {
    ...
    public void navigateToAdminRoles() {
        ...
        WebElement roles = wait.until(ExpectedConditions.elementToBeClickable(
                By.xpath("//a[@href='/admin/roles' and normalize-space(text())='Roles']")));
        roles.click();
    ...
    public void navigateToNewRole() {
        ...
        WebElement addRoleName = wait.until(ExpectedConditions.elementToBeClickable(
                By.xpath("//input[@placeholder='Enter role name']")));
        addRoleName.sendKeys("Tester"+ System.currentTimeMillis() % 100000);
        System.out.println("Role name entered");

        // Dropdown selection
        WebElement parentRoleDropdown = wait.until(
                ExpectedConditions.elementToBeClickable(By.xpath("//select[@name='parentId']")));
        ...
        List<WebElement> dropdownOptions = wait.until(
                ExpectedConditions.visibilityOfAllElementsLocatedBy(
                        By.xpath("//select[@name='parentId']/option")
                )
        );
       
        Assert.assertTrue(dropdownOptions.size() > 1,
                "Dropdown does not have enough options to skip the first one!");
        ...
        Assert.assertNotEquals(randomOption.getText().trim(), "None",
                "First option (None) was incorrectly selected!");
        try{
   		 Thread.sleep(3000);
   		} 
   	 catch(InterruptedException e) 
    ...
    public void selectRoleToUpdate() {
        List<WebElement> roles = driver.findElements(
                By.xpath("//div[@id='main-wrapper']//div[@class='col-6']/div")
        );
     ...
     //edit Checking checkboxes
        ...
        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);",wait.until(ExpectedConditions.elementToBeClickable(By.name("admin"))) );
        wait.until(ExpectedConditions.elementToBeClickable(By.name("admin"))).click();
        WebElement updateChangesBtn = wait.until(ExpectedConditions.elementToBeClickable(
                By.xpath("//button[contains(@class,'btn') and contains(@class,'btn-primary') and normalize-space(text())='Update changes']")));
        updateChangesBtn.click();
        System.out.println("changes updated");
     // ✅ Assert success message
        WebElement successMsg = wait.until(ExpectedConditions.visibilityOfElementLocated(
                By.xpath("//div[contains(text(),'Role updates successfully.')]")));
    ...
    public void selectRoleToDelete() {
        ...
        WebElement lastRoleDelete = wait.until(ExpectedConditions.elementToBeClickable(
                By.xpath("//div[@class='modal-footer']//button[@class='btn btn-danger' and normalize-space(text())='Delete']")));
